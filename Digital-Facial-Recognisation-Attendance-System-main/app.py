...
def train_model_route():
    ...
    t = threading.Thread(target=train_model_background, args=(DATASET_DIR, lambda p,m: write_train_status({"running": p < 100, "progress": p, "message": m})))
    t.daemon = True
    t.start()
...
def recognize_face():
    ...
    try:
        ...
        if emb is None:
            app.logger.info("recognize_face: no face detected by haarcascade")
